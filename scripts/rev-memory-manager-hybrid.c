...
void gatesInit() {
	AllGates = malloc(_TOTAL_GATES * sizeof(size_t));
...
unsigned memHeapNewQubits(unsigned num_qbits, qbitElement_t *res) {
  ...
  if (debugRevMemHybrid)
	...
	// malloc new qubits!
	qbit_t *new = (qbit_t *)malloc(sizeof(qbit_t)*num_qbits);
	if (new == NULL) {
...
unsigned memHeapFree(unsigned num_qbits, unsigned heap_idx, qbit_t **ancilla) {
	...
	for (size_t i = 0; i < num_qbits; i++) {
		...
		qbitElement_t *toFree = malloc(sizeof(qbitElement_t));
