...
for f in $*; do
	b=$(basename $f .scaffold)
	mkdir ${b}
	cp original.ll ${b}/${b}.ll
	cp original.ll ${b}/${b}.orig.ll
done
...
for f in $*; do
  ...
  echo "[gen-rev-mem-optimized.sh] Compiling frequency-estimation-hybrid.c" >&2
  $CLANG -c -O1 -emit-llvm $DIR/frequency-estimation-hybrid.c -o $DIR/frequency-estimation-hybrid.bc
  echo "[gen-rev-mem-optimized.sh] $b: Frequency count ..." >&2
  if [ -n ${b}/${b}.freq ]; then
    ...
    echo -e "\t[gen-rev-mem-optimized.sh] Linking frequency-estimation-hybrid.bc and ${b}/${b}_rolled.ll" >&2
