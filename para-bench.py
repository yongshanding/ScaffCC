...
def build_fun(i, all_calls, all_qs, outf, call_lists, nloop):
    ...
    if (len(callees) == 0):
        ...
        for (j, c) in enumerate(callees):
            ...
            a_end = all_qs[i][2][j+1]
            callee_q = all_qs[i][1][a_start : a_end]
...
def printStructure(call_lists):
    print("[para-bench.py] Program structure:")
...
def rand_synth(outf, nq, na, ng, nl, nd, nloop):
    ...
    random.seed(SEED)
    print("[para-bench.py] Random seed used: " + str(SEED))
    ...
    all_qs = [(0,[],[]) for _ in range(num_funs+1)]
    #all_callees = []
    #all_subs = []
    for (i, calls) in enumerate(call_lists):
        #print all_calls[i][2]
        if (i == 0 or len(calls)==0):
            nqa_parent = nq
        else:
            nqa_parent = min(nq, all_calls[i][2][0]+all_calls[i][2][1])
        n_calls = len(calls)
        #cut_calls = [0] + sorted(random.sample([k for k in range(nqa_parent)], n_calls))
        if (n_calls == 0) :
            each_size = nqa_parent
        else:
            each_size = int(nqa_parent / n_calls)
        cut_calls = [k*each_size for k in range(n_calls+1)]
        num_pass_down = cut_calls[n_calls]
        all_pass_down = random.sample([k for k in range(nqa_parent)], num_pass_down)
        for (j, c) in enumerate(calls):
            ...
            if (i == 0):
                if (len(calls) == 1):
                    ...
                    subq = cut_calls[j+1] - cut_calls[j]
        ...
        #print i
        all_qs[i] = (i, all_pass_down, cut_calls)
