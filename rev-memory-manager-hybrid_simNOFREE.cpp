...
#define _OPTE 8
#define _OPTF 9
#define _OPTG 10 //for nisq-ft
#define _OPTH 11 //for nisq
#define _OPTI 12 //for ft
...
int systemSize = 16; // perfect square number
int systemWidth = 4; 
int systemHeight= 4; 
...
void mapsInit(int num_qbits) {
    // Just set up a label (address) for each qubit id
    // malloc new qubits!
	...
	for (size_t i = 0; i < num_qbits; i++) {
		...
		//qubitsAdd(&newt[i]);
		logicalPhysicalMap.insert(make_pair(&newt[i],i));
		physicalLogicalMap.insert(make_pair(i,&newt[i]));
    }
...
void generateRectGrid(int width, int height){
	//int gridLength = std::ceil(std::sqrt(num));
	// int length = gridLength;
	initializeConnections(width*height);
	for (int i = 0; i < width*height; i++){
		...
		if (i+width < width*height)
			neighborSets[i].push_back(i+width);
		if (i-width >= 0)
			neighborSets[i].push_back(i-width);
	}
}
...
void readDeviceDescription(std::string deviceName){
    if (systemSize == 20) {
        generateRectGrid(systemWidth, systemHeight);
        return;
    }
	if (systemSize != -1){
...
vector<pair<int,int> > manhattan(qbit_t *src, qbit_t *dst){
	...
	int width = systemWidth;
	int height= systemHeight;
	int sideLength = systemWidth;
...
void printSwapChain(vector<pair<int,int> > swaps){
	for (int i = 0; i < swaps.size(); i++){
		...
		std::cerr << "SWAP q" << swaps[i].first << ", q" << swaps[i].second << "\n";
	}
}
...
int memHeapClosestQubits(int num_qbits, memHeap_t *M, qbitElement_t *res, qbit_t **inter, int targets_size) {
	...
	if ( ((salsa_p > 1) && (salsa_counter - 2) % 8 < 2 && (salsa_counter <= (4*(2*salsa_p - 1)-1))) ||
		...
		for (size_t i = 0; i < num_qbits; i++) {
			...
			logicalPhysicalMap[res[i].addr] = physicalIDs[i];
			physicalLogicalMap[physicalIDs[i]] = res[i].addr;
	...
	if (current_node->on_off == -1){
		...
		if (freePolicy == _EAGER) {
			...
			if (current_node->degree == 1){
				...
				num_younger_sis = current_node->degree;
				if (current_node->parent->is_root == 1){
				workload_1 = (nGate1 - nGate0 + swap_sum) * q_active;
				workload_0 = (nAnc + c_nAnc) * ((num_younger_sis) * ng1_avg) * weight_q * increased_weight;
				} else {
		...
		} else if (freePolicy == _OPTI) { //for braiding
			int target_num_q = systemSize;
			int threshold = 1;
			int weight_q = 1;
			int total_q = AllQubits->N;
			...
			if (current_node->is_root == 1){
				...
				if (current_node->parent->is_root == 1){
				workload_1 = (nGate1 - nGate0) * q_active;
				workload_0 = (nAnc + c_nAnc) * (num_younger_sis  * ng1_avg);
				} else {
				workload_1 = std::pow(2, current_node->level - 1) * (nGate1 - nGate0 ) * q_active;
				workload_0 = (nAnc + c_nAnc) * (num_younger_sis  * ng1_avg + current_node->parent->ng0 + nGate0);
				}
				if (workload_1 < workload_0){
					current_node->on_off = 1;
				} else {
